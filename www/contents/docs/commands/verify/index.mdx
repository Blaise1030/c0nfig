---
title: verify
description: The verify command fetches a remote configuration file over HTTP or HTTPS and validates its structure against the Kommander schema. This is useful for testing configurations before publishing them to ensure they are correctly formed.
---

## Usage

```bash
npx kmdrr@latest verify  <remote-config-url>
```
...
```bash
npx kmdrr@latest verify https://example.com/invalid-config.json
```
