...
type TCommand = { id: string, text?: string, typing?: boolean, delay?: number, clear?: boolean }

const CLISimulator = ({ commands, defaultDelay = 100, onNextCommand }: { onNextCommand: (c: TCommand) => void, commands: TCommand[], defaultDelay: number }) => {
  ...
  useEffect(() => {
    if (onNextCommand)
      onNextCommand(commands[index])
  }, [index])

  useEffect(() => {
    if (index < commands.length) {
      const { text = '', typing = true, delay = defaultDelay, clear } = commands[index];
  ...
  return (
    <div className='font-mono text-sm whitespace-pre-wrap min-w-[500px]'>
      {displayedText}
      {index < commands.length && commands[index].typing && <span className="text-blue-800 dark:text-blue-500">|</span>}
    </div>
  );
};
